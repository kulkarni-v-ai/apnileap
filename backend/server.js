...
const atlassianAuthRoutes = require("./routes/atlassianAuth");
app.use("/api/integrations/atlassian", atlassianAuthRoutes);
  ...
  try {
    ...
    ;(async () => {
      try {
        ...
        if (campus.live) {
          try {
            const jiraUsers = await jiraService.getAssignableUsers('AK');
            if (Array.isArray(jiraUsers)) {
              jiraUsers.forEach(u => { if (u.emailAddress) notifyCoordinators.add(u.emailAddress.toLowerCase().trim()); });
            }
          } catch (err) {
...
app.get("/api/cohort-stats", async (req, res) => {
  try {
    ...
    const allProjects = await prisma.project.findMany({ include: { allocations: true } });

    const stats = await Promise.all(CAMPUS_CONFIG.map(async (campus) => {
      const studentCount = await prisma.user.count({
        where: {
          persona: campus.persona,
          role: { notIn: ['COORDINATOR', 'MENTOR', 'FACULTY', 'Spoke Coordinator', 'Faculty Mentor'] }
        }
      });
      ...
      const mentorCount = await prisma.user.count({
        where: {
          persona: campus.persona,
          role: { in: ['COORDINATOR', 'MENTOR', 'FACULTY', 'Spoke Coordinator', 'Faculty Mentor'] }
        }
      });
