...
import AtlassianIntegration from "./components/AtlassianIntegration";
import {
  DragDropContext,
  Droppable,
  Draggable,
...
function RovoAgentWidget({ sessionUser, currentBoardId, activeWorkspace }) {
  ...
  const handleSend = async (text) => {
    ...
    try {
      const res = await axios.post("/api/rovo/chat", {
        prompt: text,
...
function App() {
  ...
  const handleIngestProjectSubmit = async (e) => {
    ...
    setIsIngesting(true);
    try {
      const response = await axios.post("/moderator/projects", {
  ...
  const handleUpdateProjectSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`/moderator/projects/${editingProject.id}`, {
        company: editCompany,
  ...
  const handleDeleteProject = async (projectId) => {
    ...
    try {
      const response = await axios.delete(`/moderator/projects/${projectId}`);
      if (response.data && response.data.success) {
  ...
  const fetchCampusMembers = async (boardId) => {
    try {
      const res = await axios.get(`/campuses/${boardId}/members`);
  ...
  const fetchCampusTeams = async (boardId) => {
    ...
    try {
      const res = await axios.get(`/api/teams?boardId=${boardId}`);
  ...
  const handleAcceptProject = async (projectId) => {
    ...
    try {
      const res = await axios.post(`/campus/project/${projectId}/accept`, { targetBoardId: currentBoardId });
      if (res.data && res.data.success) {
  ...
  const handleDeclineProject = async (projectId) => {
    setIsRespondingToProject(true);
    try {
      const res = await axios.post(`/campus/project/${projectId}/decline`, { targetBoardId: currentBoardId });
      if (res.data && res.data.success) {
  ...
  const handleUpdateSubmissionStatus = async (subId, newStatus, coordinatorFeedback, grade = "") => {
    try {
      const res = await axios.put(`/submissions/${subId}/status`, {
        status: newStatus,
  ...
  const handleDeleteSubmission = async (subId) => {
    try {
      const res = await axios.delete(`/submissions/${subId}`);
      if (res.data && res.data.success) {
        ...
        {(isLoading && !["hub", "moderator", "meetings", "playground", "b2b-sponsor", "faculty-mentor"].includes(activeWorkspace)) ? (
          ...
          <CorporateSponsorDashboardView
            ...
            onSubmitProposal={async (payload) => {
              try {
                const res = await axios.post("/moderator/projects", {
...
function CorporateSponsorDashboardView({ projects, loading, onRefresh, onSubmitProposal, triggerToast, sessionUser, campuses, tasks, meetings = [] }) {
  ...
  const handleAssignCompanyMentor = async (projectId, campusId, mentorId) => {
    if (!mentorId) return;
    try {
      const res = await axios.post(`/api/project/${projectId}/campus/${campusId}/project-mentor`, { mentorId });
      if (res.data && res.data.success) {
  ...
  useEffect(() => {
    const fetchCompanyMentors = async () => {
      try {
        const response = await axios.get(`/api/companies/${companyName}/mentors`);
  ...
  const handleEvaluate = async (teamId) => {
    ...
    try {
      const res = await axios.put(`/api/teams/${teamId}/evaluate`, {
        rating: Number(r),
        companyFeedback: f,
        companyGrade: g,
...
function MeetingsPortalView({ meetings, loading, onRefresh, campuses, triggerToast, moderatorProjects = [], currentPersona = "moderator" }) {
  ...
  const handlePostMeetingNotes = async () => {
    ...
    setNotesResult(null);
    try {
      const res = await axios.post(`/meetings/${notesModalMeeting.id}/notes`, { notes: notesText });
      if (res.data.success) {
  ...
  const handleGenerateNotes = async () => {
    ...
    try {
      const res = await axios.post(`/meetings/${notesModalMeeting.id}/generate-notes`);
      if (res.data.success) {
  ...
  const handleAutoProcess = async (meetId) => {
    ...
    try {
      const res = await axios.post(`/meetings/${meetId}/auto-process`);
      if (res.data.success) {
  ...
  const handleDeleteMeeting = async (meetId) => {
    ...
    try {
      const res = await axios.delete(`/meetings/${meetId}`);
      if (res.data && res.data.success) {
...
function ProjectManagerDashboardView({ projects = [], loading, onRefresh, triggerToast, campuses = [], onDeleteProject }) {
  ...
  useEffect(() => {
    ...
    const fetchCampusMentors = async () => {
      ...
      await Promise.all(campuses.map(async (campus) => {
        try {
          const res = await axios.get(`/api/campuses/${campus.id}/mentors`);
  ...
  useEffect(() => {
    ...
    const fetchCompanyMentors = async () => {
      ...
      await Promise.all(companies.map(async (company) => {
        try {
          const res = await axios.get(`/api/companies/${company}/mentors`);
  ...
  const handleAssignFacultyMentor = async (projectId, campusId, mentorId) => {
    if (!mentorId) return;
    try {
      const res = await axios.post(`/api/project/${projectId}/campus/${campusId}/faculty-mentor`, { mentorId });
      if (res.data && res.data.success) {
...
function FacultyMentorDashboardView({ 
  ...
  const fetchMentorData = async () => {
    ...
    try {
      ...
      const [projectsRes, teamsRes, studentsRes, mentorsRes] = await Promise.all([
        axios.get(`/api/mentors/${mentorId}/projects`),
        axios.get(`/api/teams?mentorId=${mentorId}`),
        axios.get(`/api/campuses/${campusId}/students`),
        axios.get(`/api/campuses/${campusId}/mentors`)
      ]);
  ...
  const handleSubmitFinalProgressClick = async (teamId) => {
    ...
    try {
      const res = await axios.put(`/api/teams/${teamId}/final-progress`, {
        reportUrl,
        grade,
        facultyComments
      });
