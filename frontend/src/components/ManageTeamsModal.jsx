...
export const ManageTeamsModal = ({ allocation, onClose, onRefresh }) => {
  ...
  const handleCreateTeam = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiClient.post(`/allocations/${allocation.id}/teams`, {
        name: newTeamName
      });
      if (res.data.success) {
  ...
  const openStudentAssignment = async (team) => {
    ...
    try {
      const res = await apiClient.get(`/students/${allocation.targetCampusId}`);
  ...
  const handleSaveAssignments = async (teamId) => {
    setAssigning(true);
    try {
      const res = await apiClient.post(`/teams/${teamId}/assign-students`, {
