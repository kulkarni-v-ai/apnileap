...
const DocumentsView = ({ currentUser, documentType = 'project' }) => {
  ...
  const fetchContextData = async () => {
    try {
      if (currentUser.role === 'STUDENT') {
        const res = await apiClient.get(`/students/${currentUser.id}/projects`);
        setAllocations(res.data);
      } else if (currentUser.role === 'MENTOR') {
        const res = await apiClient.get(`/faculty/${currentUser.id}/mentored`);
        setAllocations(res.data);
      } else {
         const campusQuery = (currentUser.role === 'SPONSOR' && currentUser.campusId) ? `?campusId=${currentUser.campusId}` : '';
         const res = await apiClient.get(`/projects${campusQuery}`);
         setProjects(res.data);
         const allocRes = await apiClient.get(`/allocations${campusQuery}`);
  ...
  const handleReview = async (id, status) => {
      ...
      try {
          await apiClient.put(`/api/documents/${id}/review`, { status, feedback });
          fetchDocuments();
      } catch(err) {
                  ...
                  {documents.filter(doc => documentType === 'project' ? doc.projectId : doc.allocationId).map(doc => (
                      <div key={doc.id} style={{ background: 'var(--bg-sidebar)', borderRadius: '8px', padding: '16px', border: '1px solid var(--border-glass)' }}>
                          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
                             ...
                             <a href={`${import.meta.env.VITE_API_BASE_URL || ''}${doc.filepath}`} target="_blank" rel="noreferrer" style={{ color: 'var(--text-muted)', cursor: 'pointer' }}>
                                 <Download size={18} />
                             </a>
                          </div>
